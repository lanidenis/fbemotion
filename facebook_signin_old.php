...
<script>
  ...
  function getComments(c_url) { //LEAST RECENT COMMENT AT THE FRONT OF LIST
	...
	if (commentList.length === 0) { result = null; }
      ...
      alert(textStatus + " This error in getComments with url: " + c_url);
      result = null;
    });
  ...
  function getLikes(url) {
    ...
    request.fail(function (jqXHR, textStatus) {
      alert(textStatus + " This error in getLikes with url: " + url);
      result = null;
    }); 
  ...
  function getReactions(url) {
    ...
    request.fail(function (jqXHR, textStatus) {
      alert(textStatus + " This error in getReactions with this url: " + JSON.stringify(url));
      result = null;
    });
  ...
  function extractData(t_url) { 
    ...
    request.done(function (response) {
        ...
        if (!response || response.error) {
          ...
          console.log("inside extractData5");
  	  ...
  	  list.extend(temp);    
          ...
          if ("paging" in response) { //sanity check
            var paging = response.paging;
            if ("next" in paging) {
              console.log("about to get another page of data!");
              var url = 'https://graph.facebook.com/v2.8' + paging["next"].substr(31);
 	  ...
 	  console.log("inside extractData6");
    ...
    if (response.status === 'connected') {
      ...
      if (finalList != null) {
        if (finalList.length != 0) {
          printData(finalList);
          document.getElementById('status').innerHTML = 'All Done! Now you can see what your post history looks like.<br>';
        }
        else {
...
<div class="container text-center" id="error">
  <div class="row">
    <div class="col-md-12">
      ...
      <p id="lead">Towards the end of this process, <br> if your data does not successfully appear, please <a href='home.html'> Try Again. </a><br><br><b>This process can take up to 10 minutes.  <br>Please leave this tab open until your posts appear in the table below.</b>
      </p>
      ...
      <br>
      This app will display your Facebook post history.<br><br> 
      </p>
